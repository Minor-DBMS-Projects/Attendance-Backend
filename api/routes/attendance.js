...
router.post("/submit", auth, async (req, res, next) => {
  ...
  let body = req.body
  let roll_nums = [];
  let attendanceData = [body.classType.toString().charAt(0), body.subject_code.toString().substring(0, 5), parseInt(body.class_id), new Date().toISOString().slice(0, 10).replace('T', ' '), userdata.id];
...
async function insertOnlineRecord(details, names) {
  ...
  let roll_nums = [];
  try {
    let presentList = await db.query(`select a.roll_no, a.class_id from (select name, roll_no, class_id from student where class_id=(select id from class where (batch='${details.batch}' AND program_id='${details.program}' AND class_group='${details.section}'))) as a  WHERE a.name in (?)`, [names]);
    let attendanceData = [details.classType.toString().charAt(0), details.subject.toString().substring(0, 5), parseInt(presentList[0].class_id), new Date().toISOString().slice(0, 10).replace('T', ' '), userdata.id];
...
router.get("/all/:classId/:subjectCode/:classType", auth, async (req, res, next) => {
  ...
  try {
    ...
    res.json({ 'records': attendanceList, 'students': students,'details': details })
  }
  catch (err) {
    console.log(err);
  }
...
router.get("/edit/:_id", auth, async (req, res) => {
  ...
  let q1 = `SELECT student.roll_no, student.class_id FROM (select * from (select * from attendanceDetails where id=${parseInt(req.params._id)}) as a JOIN attendance on a.id=attendance.details_id ) as b JOIN student on (student.class_id= b.class_id AND student.roll_no = b.roll_no ) `;
  let roll_list = [];
  try {
    ...
    let result2 = await db.query(`SELECT name, roll_no from student where class_id = ${presentRoll[0].class_id}`);
...
router.post("/edit/:_id", auth, async(req, res) => {
  ...
  try {
    await db.query(`DELETE FROM attendance WHERE details_id = ${req.params._id}`);
    await db.query(`Insert into attendance (details_id, roll_no) values ?`, [roll_nums]);
    res.redirect('/');
...
router.get("/delete/:_id", auth, async (req, res) => {
  ...
  try {
    await db.query(`DELETE FROM attendance WHERE details_id = ${parseInt(req.params._id)}`);
    await db.query(`DELETE FROM attendanceDetails WHERE id = ${parseInt(req.params._id)}`);
...
router.get("/deleteAll/:class/:subject/:type", auth, async (req, res) => {
  let sql = `select id FROM attendanceDetails WHERE (classType='${req.params.type}' AND subject_code='${req.params.subject}' AND class_id=${req.params.class} AND instructor_id=${parseInt(user)})`
  try {
    let result = await db.query(sql)
    result.forEach(async (record) => {
      await db.query(`DELETE FROM attendance WHERE details_id = ${record.id}`)
      await db.query(`DELETE FROM attendanceDetails WHERE id = ${record.id}`)
